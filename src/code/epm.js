...
var fs = require('fs');
const {mockPath, deleteMock, getMockTargetData} = require('../mock/mockFun')
...
Emp.prototype.getResponseData = function (option) {
  if (typeof option === 'string') {
    option = {
      url: option
    }
  }
  const {url, requestData = {}} = option
  const mockData = this.getMockData(url)
  return getMockTargetData(requestData, mockData.data)
}

Emp.prototype.getSelfResponseData = function () {
  const {body, query} = this.requestData
  const mockData = this.getMockData(this.url)
  return getMockTargetData(Object.assign(body, query), mockData.data)
}
